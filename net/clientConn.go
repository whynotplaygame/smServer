...
func (c *ClientConn) waitHandHhake() bool {
	...
	// 万一程序出现问题，超时了一直响应不到
	if !c.handshake {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second) //维护goroutine中的信息，可以用作超时设置
		defer cancel()
		select {
		case _ = <-c.handshakeChan:
			log.Println("client conn handshake done")
			return true
		case <-ctx.Done(): // 如果已经到了5秒钟
			log.Println("client conn handshake timeout")
			return false
		}
	}
	return true
...
func (c *ClientConn) wsReadloop() {
	...
	for {
		...
		if err == nil {
			...
			fmt.Println("客户端传来的消息的key in client conn read", key, "time", time.Now().Unix())
			// 客户端传过来的的数据是加密，需要解密
...
func (c *ClientConn) Push(name string, data interface{}) {
	...
	// c.outChan <- rsp
	// fmt.Println(rsp)
	log.Println("往管道里放的响应消息：", rsp)
	c.write(rsp.Body)
}

func (c *ClientConn) write(body interface{}) error {
	log.Println("写消息write in client conn:", body)
	// 1,把数据转成json
	...
	if err == nil {
		...
		println("secretKey in client conn,write:", key)
	}
